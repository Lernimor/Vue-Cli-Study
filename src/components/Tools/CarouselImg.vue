<template>
    <div class="carouselContain">
        <v-touch class="imgPenel animation" v-on:swipeleft="next_pic()" v-on:swiperight="prev_pic()" :swipe-options="{direction: 'horizontal'}">
        ...
        <div class="btns">
            <div v-for="(item,index) in imgUrls" :key="item">
                <span class="to_picbtn" :class="index == 0 ? 'on' : ''" @click="to_pic(index)">
                </span>
            </div>
        </div>
